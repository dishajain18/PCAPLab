...
__global__  void  mykernel(char* str,int len,char* key,int keylen,int * wordindex,int * count)
{
    int gtid = threadIdx.x;
    int i = wordindex[gtid];
    int j = 0;
    int match = 1;
...
int main()
{
    ...
    mykernel<<<1,wordcount>>>(d_str,len,d_key,keylen,d_wordindex,d_count);
